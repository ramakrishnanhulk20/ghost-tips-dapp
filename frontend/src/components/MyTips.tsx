...
export default function MyTips({ contract }: { contract: Contract | null }) {
  ...
  if (myJars.length === 0) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-sm text-gray-500 mt-2">Create one in the Send Tips tab!</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedJar && (
        <div>
          <h3 className="text-lg font-bold text-white mb-4">Tips Received (Jar #{selectedJar})</h3>
                ...
                <div key={index} className="bg-gray-800 p-4 rounded-lg border border-gray-700">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-sm font-mono text-purple-400">{tip.encryptedAmount.slice(0, 10)}...</p>
                    </div>
                  </div>
                  ...
                  <p className="text-xs text-gray-500 mt-2">{new Date(tip.timestamp * 1000).toLocaleString()}</p>
                </div>
              ))}
            </div>
          )}
